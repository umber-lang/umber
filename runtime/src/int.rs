...
impl BlockPtr {
    pub fn expect_int(self) -> i64 {
        self.expect_block().expect_int()
    }

    pub fn new_int(x: i64) -> BlockPtr {
        unsafe { Self::new(KnownTag::Int as u16, [mem::transmute(x)]) }
    }
}

#[no_mangle]
pub extern "C" fn umber_int_add(x: BlockPtr, y: BlockPtr) -> BlockPtr {
    BlockPtr::new_int(x.expect_int() + y.expect_int())
}
...
pub extern "C" fn umber_int_pow(base: BlockPtr, exp: BlockPtr) -> BlockPtr {
    BlockPtr::new_int(base.expect_int().pow(exp.expect_int() as u32))
}
...
mod test {
    ...
    fn check(f: extern "C" fn(BlockPtr, BlockPtr) -> BlockPtr, x: i64, y: i64, expect: i64) {
        assert_eq!(
            f(BlockPtr::new_int(x), BlockPtr::new_int(y)).expect_int(),
            expect
        );
    }
    ...
    fn string_conversion() {
        unsafe { umber_gc_init() };
        assert_eq!(umber_int_to_string(BlockPtr::new_int(0)).expect_str(), "0");
        assert_eq!(
            umber_int_to_string(BlockPtr::new_int(-1)).expect_str(),
            "-1"
        );
        assert_eq!(
            umber_int_to_string(BlockPtr::new_int(10)).expect_str(),
            "10"
        );
        assert_eq!(
            umber_int_to_string(BlockPtr::new_int(i64::MIN)).expect_str(),
            "-9223372036854775808"
        );
        assert_eq!(
            umber_int_to_string(BlockPtr::new_int(i64::MAX)).expect_str(),
            "9223372036854775807"
        );
    }
}
