...
let lexeme = Sedlexing.lexeme >> Ustring.of_array_unsafe
...
let line_and_pos lexbuf =
  let _, info = Sedlexing.lexing_positions lexbuf in
  info.pos_lnum, info.pos_cnum - info.pos_bol + 1
;;
...
let syntax_error ?msg lexbuf =
  ...
  in
  raise (Syntax_error (line_and_pos lexbuf, msg))
;;
...
(*let prefix_matches lexbuf s =
  ...
  let rec loop lexbuf s i =
    ...
    else match Sedlexing.next lexbuf with
      | Some c ->
        if Uchar.equal c (Ustring.get s i) then loop lexbuf s (i + 1)
        else (
          ignore (Sedlexing.backtrack lexbuf : int);
          false)
...
let read =
  ...
  and read_inline t lexbuf =
    ...
    | ".)" ->
      Sedlexing.rollback lexbuf;
      ignore (Sedlexing.next lexbuf : Uchar.t option);
