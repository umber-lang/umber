...
%token <float> FLOAT
%token <Uchar.t> CHAR
%token <Umber_common.Ustring.t> STRING

%token UNDERSCORE
%token <Umber_common.Ustring.t> LOWER_NAME
%token <Umber_common.Ustring.t> UPPER_NAME
%token <Umber_common.Ustring.t> OPERATOR

%token EOF

%start <Untyped_ast.Module.t> prog
...
stmt_common:
  ...
  | EFFECT; name = UPPER_NAME; params = type_params;
    ...
    { Module.Effect (
        ...
        Untyped_ast.create_effect (Type_decl.params_of_list params) sigs) }
