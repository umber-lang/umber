...
(* TODO: This file is too long. Split it up. *)
...
module Effect_branch = struct
  ...
  let of_untyped_with_names
    ...
    ~result_type
    pat_names
    ({ operation; args } : Untyped_ast.Effect_pattern.t)
    =
    ...
    let rec of_untyped ~names ~types ~f_name expr
      ...
      let result =
        Node.with_value expr ~f:(fun expr ->
          match (expr : Untyped_ast.Expr.t) with
          | Literal lit ->
            ( node (Literal lit)
            , Type_bindings.instantiate_type_scheme ~names ~types (Literal.typ lit)
          ...
          | Op_section { op_side; op; expr } ->
            ...
            let rewritten_expr : Untyped_ast.Expr.t =
              Let
                { rec_ = false
                ; bindings =
                    [ ( Node.create
                          (Untyped_ast.Pattern.catch_all (Some applied_arg_var))
                          expr_span
                      , None
                      , expr )
                    ]
                ; body =
                    Node.create
                      (Untyped_ast.Expr.Lambda
                         ( [ Node.create
                               (Untyped_ast.Pattern.catch_all (Some unapplied_arg_var))
                               op_span
                           ]
                         , Node.create
                             (Untyped_ast.Expr.Fun_call
                                ( Node.map op ~f:Untyped_ast.Expr.name
                                , [ Node.create
                                      (Untyped_ast.Expr.Name (qualified left_var))
                                      left_var_span
                                  ; Node.create
                                      (Untyped_ast.Expr.Name (qualified right_var))
                                      right_var_span
                                  ] ))
                             expr_span ))
          ...
          | Match_function branches ->
            ...
            of_untyped
              ...
              (node
                 (Untyped_ast.Expr.Lambda
                    ( [ node (Untyped_ast.Pattern.catch_all (Some name)) ]
                    , node
                        (Untyped_ast.Expr.Match
                           ( node
                               (Untyped_ast.Expr.Name (Module_path.Relative.empty, name))
                           , branches )) )))
          ...
          | Seq_literal items ->
            ...
            let desugared_expr =
              List.fold_right items ~init:(make_constructor "Nil") ~f:(fun item expr ->
                Node.create
                  (Untyped_ast.Expr.Fun_call (make_constructor "Cons", [ item; expr ]))
...
let untyped_expr_names_used ~names expr =
  let add_locals init = Untyped_ast.Pattern.fold_names ~init ~f:Set.add in
  let rec loop ~names used locals (expr : Untyped_ast.Expr.t) =
    match expr with
    | Literal _ -> used
    ...
    | Let { rec_; bindings; body } ->
      let new_locals =
        Nonempty.fold
          bindings
          ~init:locals
          ~f:(fun locals (pat, (_ : Fixity.t option), (_expr : Untyped_ast.Expr.t Node.t))
  ...
  and loop_record_fields ~names used locals =
    Nonempty.fold ~init:used ~f:(fun used ((_field_name : Value_name.t), expr) ->
      ...
      | None -> used)
  in
  loop ~names Value_name.Absolute.Set.empty Value_name.Set.empty expr
;;
...
module Module = struct
  ...
  module Sig_data : sig
    ...
    val add_type_decl
      ...
      -> def:Untyped_ast.Module.def Node.t
      -> t
    ...
    val fold_defs
      :  t
      -> init:'acc
      -> f:('acc -> Untyped_ast.Module.def Node.t -> 'acc)
      -> 'acc
  end = struct
    type t =
      { type_decls : Untyped_ast.Module.def Node.t Type_name.Map.t
      ; effect_decls : Untyped_ast.Module.def Node.t Effect_name.Map.t
      }
    [@@deriving fields]
              ...
              Node.with_value pat ~f:(fun pat ->
                Name_bindings.merge_names
                  ...
                  (Pattern_names.gather
                     ...
                     ~type_source:Placeholder
                     ~fixity
                     ~fold:Untyped_ast.Pattern.fold_names)
  ...
  module Intermediate = struct
    type binding =
      (Untyped_ast.Pattern.t * Pattern_names.t) Node.t
      * Fixity.t option
  ...
  let rec handle_value_bindings
    ...
    sigs
    (defs :
      (Untyped_ast.Let_binding_group.t, Module_path.absolute) Module.def Node.t list)
      ...
      Node.with_value def ~f:(function
        | Let { bindings = bindings'; rec_ } ->
          ...
          , Nonempty.fold_right
              ...
              let used_names = Node.with_value expr ~f:(untyped_expr_names_used ~names) in
