...
let rec to_untyped_expr_as_is : t -> Untyped_ast.Expr.t Node.t = function
  | Leaf expr -> expr
  | Node (op_name, left_child, right_child) ->
    ...
    Node.create
      (Untyped_ast.Expr.Fun_call
         (Node.map op_name ~f:Untyped_ast.Expr.name, [ left_arg; right_arg ]))
      span
;;
...
let%test_module _ =
  (module struct
    ...
    let dummy_expr_generator =
      Quickcheck.Generator.return (Node.dummy_span (Untyped_ast.Expr.Literal (Int 0)))
    ;;
