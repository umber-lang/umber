...
%inline def:
  ...
  | def = preceded(equals, block(nonempty_lines(stmt))) { def }
...
stmt:
  ...
  (* FIXME: this DEDENT* is a hack and shouldn't work. The fact I don't apply it to
     function definitions as well is almost certainly wrong.
     This is here because of stray indents which are ignored in block_indents causing
     DEDENTs to be generated by the lexer after leaving the tuple, seq, record, etc.
     A good option for now might just be to enforce specific indentation for those. *)
  (* FIXME: let statements (not expressions) are always recursive *)
  | LET; pat = pattern; equals; expr = expr; DEDENT* { Module.Let (pat, expr) }
  | LET; fun_name = pattern_name; args = nonempty_list(pattern_term); equals;
    expr = expr
    { let expr = List.fold_right args ~init:expr ~f:Expr.lambda in
