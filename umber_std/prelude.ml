open! Core

(*$
  open! Core

  let compile_and_print ~name ~is_sexp target =
    print_endline [%string "\nlet %{name} = {|"];
    Umberboot.compile
      ~filename:"Std/Prelude.um"
      ~no_std:true
      ~parent:(Umber.Ast.Module_name.of_string_exn "Std")
      [ target, Stdout ];
    print_endline "|}";
    if is_sexp then print_endline "|> Sexp.of_string"
  ;;
$*)

(*$ let () = compile_and_print ~name:"names" ~is_sexp:true Names *)
let names = {|
((current_path "Std(d)")
 (toplevel
  ((names
    ((True (Local ((name_id %true) (typ (Scheme (Type_app Bool ()))))))
     (False (Local ((name_id %false) (typ (Scheme (Type_app Bool ()))))))))
   (types
    ((Int ((Local (() Abstract))))
     (Bool ((Local (() (Variants ((False ()) (True ())))))))
     (Char ((Local (() Abstract)))) (Float ((Local (() Abstract))))
     (String ((Local (() Abstract))))))
   (modules
    ((Std
      (Local
       (()
        ((names ()) (types ())
         (modules
          ((Prelude
            (Local
             ((((names
                 ((* (Imported Std.Prelude.Operators.*))
                  (+ (Imported Std.Prelude.Operators.+))
                  (- (Imported Std.Prelude.Operators.-))
                  (. (Imported Std.Prelude.Operators..))
                  (";" (Imported "Std.Prelude.Operators.;"))
                  (< (Imported Std.Prelude.Operators.<))
                  (> (Imported Std.Prelude.Operators.>))
                  (^ (Imported Std.Prelude.Operators.^))
                  (!= (Imported Std.Prelude.Operators.!=))
                  (&& (Imported Std.Prelude.Operators.&&))
                  (:: (Imported Std.Prelude.Operators.::))
                  (<= (Imported Std.Prelude.Operators.<=))
                  (== (Imported Std.Prelude.Operators.==))
                  (>= (Imported Std.Prelude.Operators.>=))
                  (|> (Imported Std.Prelude.Operators.|>))
                  (|| (Imported Std.Prelude.Operators.||))
                  (Nil (Imported Std.Prelude.List.Nil))
                  (not (Imported Std.Prelude.Operators.not))
                  (Cons (Imported Std.Prelude.List.Cons))
                  (sqrt
                   (Local
                    ((name_id Std.Prelude.sqrt)
                     (typ
                      (Scheme
                       (Function ((Type_app Int ())) (Type_app Float ())))))))
                  (print
                   (Local
                    ((name_id Std.Prelude.print)
                     (typ
                      (Scheme (Function ((Type_app String ())) (Tuple ()))))
                     (type_source Extern_declared)
                     (extern_name umber_print_endline))))
                  (print_int
                   (Local
                    ((name_id Std.Prelude.print_int)
                     (typ (Scheme (Function ((Type_app Int ())) (Tuple ()))))
                     (type_source Extern_declared)
                     (extern_name umber_print_int))))))
                (types ())
                (modules
                 ((List
                   (Local
                    (()
                     ((names
                       ((Nil
                         (Local
                          ((name_id Std.Prelude.List.Nil)
                           (typ
                            (Scheme
                             (Type_app Std.Prelude.List.List ((Var a))))))))
                        (Cons
                         (Local
                          ((name_id Std.Prelude.List.Cons)
                           (typ
                            (Scheme
                             (Function
                              ((Var a)
                               (Type_app Std.Prelude.List.List ((Var a))))
                              (Type_app Std.Prelude.List.List ((Var a)))))))))))
                      (types
                       ((List
                         ((Local
                           ((a)
                            (Variants
                             ((Nil ())
                              (Cons
                               ((Var a)
                                (Type_app Std.Prelude.List.List ((Var a)))))))))))))
                      (modules ())))))
                  (Operators
                   (Local
                    (()
                     ((names
                       ((*
                         (Local
                          ((name_id Std.Prelude.Operators.*)
                           (typ
                            (Scheme
                             (Function ((Type_app Int ()) (Type_app Int ()))
                              (Type_app Int ()))))
                           (fixity (Left 7)))))
                        (+
                         (Local
                          ((name_id Std.Prelude.Operators.+)
                           (typ
                            (Scheme
                             (Function ((Type_app Int ()) (Type_app Int ()))
                              (Type_app Int ()))))
                           (fixity (Left 6)))))
                        (-
                         (Local
                          ((name_id Std.Prelude.Operators.-)
                           (typ
                            (Scheme
                             (Function ((Type_app Int ()) (Type_app Int ()))
                              (Type_app Int ()))))
                           (fixity (Left 6)))))
                        (.
                         (Local
                          ((name_id Std.Prelude.Operators..)
                           (typ
                            (Scheme
                             (Function
                              ((Function ((Var b)) (Var c))
                               (Function ((Var a)) (Var b)) (Var a))
                              (Var c))))
                           (fixity (Right 9)))))
                        (";"
                         (Local
                          ((name_id "Std.Prelude.Operators.;")
                           (typ
                            (Scheme (Function ((Tuple ()) (Var a)) (Var a))))
                           (fixity (Left 0)))))
                        (<
                         (Local
                          ((name_id Std.Prelude.Operators.<)
                           (typ
                            (Scheme
                             (Function ((Var a) (Var a)) (Type_app Bool ()))))
                           (fixity (Non_assoc 4)))))
                        (>
                         (Local
                          ((name_id Std.Prelude.Operators.>)
                           (typ
                            (Scheme
                             (Function ((Var a) (Var a)) (Type_app Bool ()))))
                           (fixity (Non_assoc 4)))))
                        (^
                         (Local
                          ((name_id Std.Prelude.Operators.^)
                           (typ
                            (Scheme
                             (Function ((Type_app Int ()) (Type_app Int ()))
                              (Type_app Int ()))))
                           (fixity (Right 8)))))
                        (!=
                         (Local
                          ((name_id Std.Prelude.Operators.!=)
                           (typ
                            (Scheme
                             (Function ((Var a) (Var a)) (Type_app Bool ()))))
                           (fixity (Non_assoc 4)))))
                        (&&
                         (Local
                          ((name_id Std.Prelude.Operators.&&)
                           (typ
                            (Scheme
                             (Function
                              ((Type_app Bool ()) (Type_app Bool ()))
                              (Type_app Bool ()))))
                           (fixity (Left 3)))))
                        (::
                         (Local
                          ((name_id Std.Prelude.Operators.::)
                           (typ
                            (Scheme
                             (Function
                              ((Var a) (Type_app Std.Prelude.List ((Var a))))
                              (Type_app Std.Prelude.List ((Var a))))))
                           (fixity (Right 5)))))
                        (<=
                         (Local
                          ((name_id Std.Prelude.Operators.<=)
                           (typ
                            (Scheme
                             (Function ((Var a) (Var a)) (Type_app Bool ()))))
                           (fixity (Non_assoc 4)))))
                        (==
                         (Local
                          ((name_id Std.Prelude.Operators.==)
                           (typ
                            (Scheme
                             (Function ((Var a) (Var a)) (Type_app Bool ()))))
                           (fixity (Non_assoc 4)))))
                        (>=
                         (Local
                          ((name_id Std.Prelude.Operators.>=)
                           (typ
                            (Scheme
                             (Function ((Var a) (Var a)) (Type_app Bool ()))))
                           (fixity (Non_assoc 4)))))
                        (|>
                         (Local
                          ((name_id Std.Prelude.Operators.|>)
                           (typ
                            (Scheme
                             (Function ((Var a) (Function ((Var a)) (Var b)))
                              (Var b))))
                           (fixity (Left 0)))))
                        (||
                         (Local
                          ((name_id Std.Prelude.Operators.||)
                           (typ
                            (Scheme
                             (Function
                              ((Type_app Bool ()) (Type_app Bool ()))
                              (Type_app Bool ()))))
                           (fixity (Left 2)))))
                        (not
                         (Local
                          ((name_id Std.Prelude.Operators.not)
                           (typ
                            (Scheme
                             (Function ((Type_app Bool ()))
                              (Type_app Bool ())))))))))
                      (types ()) (modules ())))))))))
              ((names
                ((* (Imported Std.Prelude.Operators.*))
                 (+ (Imported Std.Prelude.Operators.+))
                 (- (Imported Std.Prelude.Operators.-))
                 (. (Imported Std.Prelude.Operators..))
                 (";" (Imported "Std.Prelude.Operators.;"))
                 (< (Imported Std.Prelude.Operators.<))
                 (> (Imported Std.Prelude.Operators.>))
                 (^ (Imported Std.Prelude.Operators.^))
                 (!= (Imported Std.Prelude.Operators.!=))
                 (&& (Imported Std.Prelude.Operators.&&))
                 (:: (Imported Std.Prelude.Operators.::))
                 (<= (Imported Std.Prelude.Operators.<=))
                 (== (Imported Std.Prelude.Operators.==))
                 (>= (Imported Std.Prelude.Operators.>=))
                 (|> (Imported Std.Prelude.Operators.|>))
                 (|| (Imported Std.Prelude.Operators.||))
                 (not (Imported Std.Prelude.Operators.not))
                 (sqrt
                  (Local
                   ((name_id Std.Prelude.sqrt)
                    (typ
                     (Scheme
                      (Function ((Type_app Int ())) (Type_app Float ()))))
                    (type_source Extern_declared)
                    (extern_name umber_int_sqrt))))
                 (print
                  (Local
                   ((name_id Std.Prelude.print)
                    (typ
                     (Scheme (Function ((Type_app String ())) (Tuple ()))))
                    (type_source Extern_declared)
                    (extern_name umber_print_endline))))
                 (print_int
                  (Local
                   ((name_id Std.Prelude.print_int)
                    (typ (Scheme (Function ((Type_app Int ())) (Tuple ()))))
                    (type_source Extern_declared)
                    (extern_name umber_print_int))))))
               (types ())
               (modules
                ((List
                  (Local
                   (()
                    ((names
                      ((Nil
                        (Local
                         ((name_id Std.Prelude.List.Nil)
                          (typ
                           (Scheme
                            (Type_app Std.Prelude.List.List ((Var a))))))))
                       (Cons
                        (Local
                         ((name_id Std.Prelude.List.Cons)
                          (typ
                           (Scheme
                            (Function
                             ((Var a)
                              (Type_app Std.Prelude.List.List ((Var a))))
                             (Type_app Std.Prelude.List.List ((Var a)))))))))))
                     (types
                      ((List
                        ((Local
                          ((a)
                           (Variants
                            ((Nil ())
                             (Cons
                              ((Var a)
                               (Type_app Std.Prelude.List.List ((Var a)))))))))))))
                     (modules ())))))
                 (Operators
                  (Local
                   (()
                    ((names
                      ((*
                        (Local
                         ((name_id Std.Prelude.Operators.*)
                          (typ
                           (Scheme
                            (Function ((Type_app Int ()) (Type_app Int ()))
                             (Type_app Int ()))))
                          (type_source Extern_declared) (fixity (Left 7))
                          (extern_name umber_int_mul))))
                       (+
                        (Local
                         ((name_id Std.Prelude.Operators.+)
                          (typ
                           (Scheme
                            (Function ((Type_app Int ()) (Type_app Int ()))
                             (Type_app Int ()))))
                          (type_source Extern_declared) (fixity (Left 6))
                          (extern_name umber_int_add))))
                       (-
                        (Local
                         ((name_id Std.Prelude.Operators.-)
                          (typ
                           (Scheme
                            (Function ((Type_app Int ()) (Type_app Int ()))
                             (Type_app Int ()))))
                          (type_source Extern_declared) (fixity (Left 6))
                          (extern_name umber_int_sub))))
                       (.
                        (Local
                         ((name_id Std.Prelude..)
                          (typ
                           (Scheme
                            (Function
                             ((Function ((Var b)) (Var c))
                              (Function ((Var a)) (Var b)) (Var a))
                             (Var c))))
                          (fixity (Right 9)))))
                       (";"
                        (Local
                         ((name_id "Std.Prelude.;")
                          (typ
                           (Scheme (Function ((Tuple ()) (Var a)) (Var a))))
                          (fixity (Left 0)))))
                       (<
                        (Local
                         ((name_id Std.Prelude.Operators.<)
                          (typ
                           (Scheme
                            (Function ((Var a) (Var a)) (Type_app Bool ()))))
                          (type_source Extern_declared)
                          (fixity (Non_assoc 4)) (extern_name umber_lt))))
                       (>
                        (Local
                         ((name_id Std.Prelude.Operators.>)
                          (typ
                           (Scheme
                            (Function ((Var a) (Var a)) (Type_app Bool ()))))
                          (type_source Extern_declared)
                          (fixity (Non_assoc 4)) (extern_name umber_gt))))
                       (^
                        (Local
                         ((name_id Std.Prelude.Operators.^)
                          (typ
                           (Scheme
                            (Function ((Type_app Int ()) (Type_app Int ()))
                             (Type_app Int ()))))
                          (type_source Extern_declared) (fixity (Right 8))
                          (extern_name umber_int_pow))))
                       (!=
                        (Local
                         ((name_id Std.Prelude.Operators.!=)
                          (typ
                           (Scheme
                            (Function ((Var a) (Var a)) (Type_app Bool ()))))
                          (type_source Extern_declared)
                          (fixity (Non_assoc 4)) (extern_name umber_neq))))
                       (&&
                        (Local
                         ((name_id Std.Prelude.&&)
                          (typ
                           (Scheme
                            (Function ((Type_app Bool ()) (Type_app Bool ()))
                             (Type_app Bool ()))))
                          (fixity (Left 3)))))
                       (::
                        (Local
                         ((name_id Std.Prelude.::)
                          (typ
                           (Scheme
                            (Function
                             ((Var a) (Type_app Std.Prelude.List ((Var a))))
                             (Type_app Std.Prelude.List ((Var a))))))
                          (fixity (Right 5)))))
                       (<=
                        (Local
                         ((name_id Std.Prelude.Operators.<=)
                          (typ
                           (Scheme
                            (Function ((Var a) (Var a)) (Type_app Bool ()))))
                          (type_source Extern_declared)
                          (fixity (Non_assoc 4)) (extern_name umber_lte))))
                       (==
                        (Local
                         ((name_id Std.Prelude.Operators.==)
                          (typ
                           (Scheme
                            (Function ((Var a) (Var a)) (Type_app Bool ()))))
                          (type_source Extern_declared)
                          (fixity (Non_assoc 4)) (extern_name umber_eq))))
                       (>=
                        (Local
                         ((name_id Std.Prelude.Operators.>=)
                          (typ
                           (Scheme
                            (Function ((Var a) (Var a)) (Type_app Bool ()))))
                          (type_source Extern_declared)
                          (fixity (Non_assoc 4)) (extern_name umber_gte))))
                       (|>
                        (Local
                         ((name_id Std.Prelude.|>)
                          (typ
                           (Scheme
                            (Function ((Var a) (Function ((Var a)) (Var b)))
                             (Var b))))
                          (fixity (Left 0)))))
                       (||
                        (Local
                         ((name_id Std.Prelude.||)
                          (typ
                           (Scheme
                            (Function ((Type_app Bool ()) (Type_app Bool ()))
                             (Type_app Bool ()))))
                          (fixity (Left 2)))))
                       (not
                        (Local
                         ((name_id Std.Prelude.not)
                          (typ
                           (Scheme
                            (Function ((Type_app Bool ()))
                             (Type_app Bool ()))))
                          (type_source Let_inferred))))))
                     (types ()) (modules ()))))))))))))))))))))))
|}
|> Sexp.of_string
(*$*)

(*$ let () = compile_and_print ~name:"llvm" ~is_sexp:false Llvm *)
let llvm = {|
; ModuleID = 'Std/Prelude.um'
source_filename = "Std/Prelude.um"
target datalayout = "i32:64-i64:64-p:64:64-f64:64"

%umber_block = type { %umber_header, [0 x i64] }
%umber_header = type { i16, i16, i32 }

@Std.Prelude.List.Nil = constant %umber_block* inttoptr (i64 1 to %umber_block*)
@"Std.Prelude.::" = constant %umber_block* bitcast (%umber_block* (%umber_block*, %umber_block*)* @Std.Prelude.List.Cons to %umber_block*)

define i32 @"mainStd/Prelude.um"() {
entry:
  ret i32 0
}

define tailcc %umber_block* @Std.Prelude.List.Cons(%umber_block* %arg0, %umber_block* %arg1) {
entry:
  %malloccall = tail call i8* @malloc(i32 mul (i32 ptrtoint (i64* getelementptr (i64, i64* null, i32 1) to i32), i32 3))
  %box = bitcast i8* %malloccall to i64*
  %box1 = bitcast i64* %box to i16*
  store i16 0, i16* %box1, align 2
  %box2 = getelementptr i16, i16* %box1, i64 1
  store i16 2, i16* %box2, align 2
  %box3 = bitcast i64* %box to %umber_block**
  %box4 = getelementptr %umber_block*, %umber_block** %box3, i64 1
  store %umber_block* %arg0, %umber_block** %box4, align 8
  %box5 = bitcast i64* %box to %umber_block**
  %box6 = getelementptr %umber_block*, %umber_block** %box5, i64 2
  store %umber_block* %arg1, %umber_block** %box6, align 8
  %box7 = bitcast i64* %box to %umber_block*
  ret %umber_block* %box7
}

define tailcc %umber_block* @Std.Prelude..(%umber_block* %Std.Prelude.Operators.f.2, %umber_block* %Std.Prelude.Operators.g.1, %umber_block* %Std.Prelude.Operators.x.3) {
entry:
  %Std.Prelude.Operators.f.21 = bitcast %umber_block* %Std.Prelude.Operators.f.2 to %umber_block* (%umber_block*)*
  %Std.Prelude.Operators.g.12 = bitcast %umber_block* %Std.Prelude.Operators.g.1 to %umber_block* (%umber_block*)*
  %fun_call = tail call %umber_block* %Std.Prelude.Operators.g.12(%umber_block* %Std.Prelude.Operators.x.3)
  %fun_call3 = tail call %umber_block* %Std.Prelude.Operators.f.21(%umber_block* %fun_call)
  ret %umber_block* %fun_call3
}

declare %umber_block* @umber_int_pow(%umber_block*, %umber_block*)

declare %umber_block* @umber_int_mul(%umber_block*, %umber_block*)

declare %umber_block* @umber_int_add(%umber_block*, %umber_block*)

declare %umber_block* @umber_int_sub(%umber_block*, %umber_block*)

define tailcc %umber_block* @Std.Prelude.not(%umber_block* %Std.Prelude.Operators..1) {
entry:
  br label %cond

cond_binding_merge:                               ; preds = %cond_binding
  br label %cond_otherwise_merge

cond:                                             ; preds = %entry
  %Std.Prelude.Operators..11 = ptrtoint %umber_block* %Std.Prelude.Operators..1 to i64
  %equals = icmp eq i64 %Std.Prelude.Operators..11, 3
  br i1 %equals, label %cond_binding, label %cond_otherwise

cond_binding:                                     ; preds = %cond
  br label %cond_binding_merge

cond_otherwise:                                   ; preds = %cond
  br label %cond_otherwise_merge

cond_otherwise_merge:                             ; preds = %cond_otherwise, %cond_binding_merge
  %cond_otherwise_merge2 = phi %umber_block* [ inttoptr (i64 1 to %umber_block*), %cond_binding_merge ], [ inttoptr (i64 3 to %umber_block*), %cond_otherwise ]
  ret %umber_block* %cond_otherwise_merge2
}

declare %umber_block* @umber_eq(%umber_block*, %umber_block*)

declare %umber_block* @umber_neq(%umber_block*, %umber_block*)

declare %umber_block* @umber_lt(%umber_block*, %umber_block*)

declare %umber_block* @umber_lte(%umber_block*, %umber_block*)

declare %umber_block* @umber_gt(%umber_block*, %umber_block*)

declare %umber_block* @umber_gte(%umber_block*, %umber_block*)

define tailcc %umber_block* @"Std.Prelude.&&"(%umber_block* %Std.Prelude.Operators.a.2, %umber_block* %Std.Prelude.Operators.b.2) {
entry:
  br label %cond

cond_binding_merge:                               ; preds = %cond_binding
  br label %cond_otherwise_merge

cond:                                             ; preds = %entry
  %Std.Prelude.Operators.a.21 = ptrtoint %umber_block* %Std.Prelude.Operators.a.2 to i64
  %equals = icmp eq i64 %Std.Prelude.Operators.a.21, 3
  br i1 %equals, label %cond_binding, label %cond_otherwise

cond_binding:                                     ; preds = %cond
  br label %cond_binding_merge

cond_otherwise:                                   ; preds = %cond
  br label %cond_otherwise_merge

cond_otherwise_merge:                             ; preds = %cond_otherwise, %cond_binding_merge
  %cond_otherwise_merge2 = phi %umber_block* [ %Std.Prelude.Operators.b.2, %cond_binding_merge ], [ inttoptr (i64 1 to %umber_block*), %cond_otherwise ]
  ret %umber_block* %cond_otherwise_merge2
}

define tailcc %umber_block* @"Std.Prelude.||"(%umber_block* %Std.Prelude.Operators.a.3, %umber_block* %Std.Prelude.Operators.b.3) {
entry:
  br label %cond

cond_binding_merge:                               ; preds = %cond_binding
  br label %cond_otherwise_merge

cond:                                             ; preds = %entry
  %Std.Prelude.Operators.a.31 = ptrtoint %umber_block* %Std.Prelude.Operators.a.3 to i64
  %equals = icmp eq i64 %Std.Prelude.Operators.a.31, 3
  br i1 %equals, label %cond_binding, label %cond_otherwise

cond_binding:                                     ; preds = %cond
  br label %cond_binding_merge

cond_otherwise:                                   ; preds = %cond
  br label %cond_otherwise_merge

cond_otherwise_merge:                             ; preds = %cond_otherwise, %cond_binding_merge
  %cond_otherwise_merge2 = phi %umber_block* [ inttoptr (i64 3 to %umber_block*), %cond_binding_merge ], [ %Std.Prelude.Operators.b.3, %cond_otherwise ]
  ret %umber_block* %cond_otherwise_merge2
}

define tailcc %umber_block* @"Std.Prelude.|>"(%umber_block* %Std.Prelude.Operators.x.4, %umber_block* %Std.Prelude.Operators.f.3) {
entry:
  %Std.Prelude.Operators.f.31 = bitcast %umber_block* %Std.Prelude.Operators.f.3 to %umber_block* (%umber_block*)*
  %fun_call = tail call %umber_block* %Std.Prelude.Operators.f.31(%umber_block* %Std.Prelude.Operators.x.4)
  ret %umber_block* %fun_call
}

define tailcc %umber_block* @"Std.Prelude.;"(%umber_block* %"*lambda_arg", %umber_block* %Std.Prelude.Operators.x.5) {
entry:
  ret %umber_block* %Std.Prelude.Operators.x.5
}

declare %umber_block* @umber_int_sqrt(%umber_block*)

declare %umber_block* @umber_print_endline(%umber_block*)

declare %umber_block* @umber_print_int(%umber_block*)

declare noalias i8* @malloc(i32)

|}
(*$*)
